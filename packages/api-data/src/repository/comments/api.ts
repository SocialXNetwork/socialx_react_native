...
export default (context: IContext) => ({
	...
	likeComment: async (likeCommentInput: { commentId: string }): Promise<null> => {
		let validatedInput: any;
		try {
	...
	removeComment: async (removeCommentInput: IRemoveCommentInput): Promise<null> => {
		let validatedInput: any;
		try {
			validatedInput = await schemas.removeComment.validate(removeCommentInput, {
				stripUnknown: true,
			});
		} catch (e) {
			throw new ValidationError(typeof e.errors === 'string' ? e.errors : e.errors.join(), {
	...
	unlikeComment: async (unlikeCommentInput: IUnlikeCommentInput): Promise<null> => {
		let validatedInput: any;
		try {
			validatedInput = await schemas.unlikeComment.validate(unlikeCommentInput, {
				stripUnknown: true,
			});
		} catch (e) {
			throw new ValidationError(typeof e.errors === 'string' ? e.errors : e.errors.join(), {
