...
Gun.chain.currentAccountRecover = function()  {
	return this.user().get(TABLE_ENUMS.RECOVER);
};
...
Gun.chain.commentsByPostPath = function( postPath: string) {
	return this.get(TABLES.POSTS).path(postPath).get(TABLE_ENUMS.COMMENTS);
}

Gun.chain.commentMetaById = function( commentId: string)  {
	return this.get(TABLES.COMMENT_META_BY_ID).get(commentId);
};

Gun.chain.likesByCommentPath = function( commentPath: string)  {
	return this.path(commentPath).get(TABLE_ENUMS.LIKES);
};

// Notifications
Gun.chain.notificationsByUsername = function(
	username: string,
)  {
...
Gun.chain.postMetasByPostIdOfCurrentAccount = function (
	postId: string,
) {
	return this
