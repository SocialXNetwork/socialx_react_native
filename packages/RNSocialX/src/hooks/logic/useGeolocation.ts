...
const useGeolocation = (): IGeoLocationSensorState => {
	const [state, setState] = useState<IGeoLocationSensorState>({
		loading: true,
		accuracy: null,
		altitude: null,
	...
	const onEventError = (error: PositionError) =>
		mounted &&
		setState((oldState: IGeoLocationSensorState) => ({ ...oldState, loading: false, error }));
