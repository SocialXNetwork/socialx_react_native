...
const useList = <T>(initialList: T[] = []): [T[], IListActions<T>] => {
	...
	return [
		list,
		{
			...
			updateAt: (index, entry) =>
				set((currentList) => [...currentList.slice(0, index), entry, ...list.slice(index + 1)]),
			remove: (index) =>
				set((currentList) => [...currentList.slice(0, index), ...list.slice(index + 1)]),
