...
const WallPostThumbs: React.SFC<IWallPostsProps> = ({
	wallPosts,
	onThumbPress,
}) => (
	<ScrollView
	...
	>
		{wallPosts.map((wallPost: IActivitySuperLikedCardPosts, index: number) => (
			<TouchableOpacity
				...
				onPress={() => onThumbPress(wallPost.postId)}
			>
				<Image
					source={{ uri: wallPost.postThumbURL }}
					resizeMode={'contain'}
					style={style.postThumbImage}
				/>
			</TouchableOpacity>
		))}
	</ScrollView>
...
export const ActivitySuperLikedCard: React.SFC<
	IActivitySuperLikedCardProps
> = ({ avatarURL, fullName, timestamp, wallPosts, onThumbPress, getText }) => {
	return (
		<View style={style.container}>
			...
			<View style={style.rightContainer}>
				<View style={style.topRightRow}>
					...
					<Text style={style.notificationTimestamp}>
						{moment(timestamp).fromNow()}
					</Text>
				</View>
				<Text style={style.activityActionText}>
					<Text style={style.pinkText}>{`${getText(
						'notifications.card.super.liked.part1',
					)} `}</Text>
